...
I registered a target group for the NLB specifically targeting the Application Load Balancer (TCP on port 80)
...
Once the Network Load Balancer was provisioned and registered with DNS, a hello world message could be accessed through the Network Load Balancer DNS address.

Ultimately, this setup demonstrates an architecture commonly used when a static IP is required for whitelisting (provided by the NLB), while still leveraging the Layer 7 routing capabilities of the ALB for the backend.
